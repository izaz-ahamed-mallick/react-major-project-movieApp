        ...
        <div className="w-full flex mt-2">
          ...
          <div className="content w-[80%] ml-[4%] text-white">
            ...
            <div className="flex items-center mt-4 mb-2 gap-4">
              ...
              <h1>
                {info.detail.first_air_date}({info.detail.status})
              </h1>
              <h1>{info.detail.genres.map((g) => g.name).join(",")}</h1>
              <h1>{info.detail.runtime || info.detail.episode_run_time} mins</h1>
            </div>
            <div>
              ...
              <p className="mb-4">
                {info.translations.map((l) => l.english_name).join(", ")}
              </p>
              <Link
                className=" bg-[#6556CD] hover:bg-[#5b49d1] duration-200 px-4 py-3 rounded-lg "
                to={`${pathname}/trailer`}
              >
                <i className="fa-solid fa-play mr-1"></i>Play Trailer
              </Link>
            </div>
          </div>
        </div>
            ...
            {info.detail.seasons.map((s, i) => {
              return (
                <div className="min-w-[27vh] relative" key={i}>
                  <img
                    className="h-[40vh] object-cover object-top-right rounded-lg shadow-[8px_17px_38px_2px_rgba(0,0,0,0.3)]"
                    src={
                    ...
                    alt=""
                  />
                  <h1 className="text-xl text-zinc-300">{s.name}</h1>
                </div>
              );
            })}
          </div>
