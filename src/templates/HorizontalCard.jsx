...
const HorizontalCard = ({ data, title }) => {
  return (
    <div className=" lg:w-[100%] flex overflow-y-hidden lg:mb-5 lg:px-5">
      {data.length > 0 ? (
        data.map((trendData, index) => {
          return (
            <Link
              to={`/${trendData.media_type || title}/details/${trendData.id}`}
              key={index}
              className=" min-w-[50vw] h-[40vh] lg:min-w-[20%] lg:max-w-[20%] lg:h-[50vh] bg-zinc-900  rounded-xl mr-5 pb-2 mb-2"
            >
              <img
